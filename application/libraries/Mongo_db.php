...
class Mongo_db
{
	...
	// ofer private $_query_safety = 'safe';
	...
	public function get_php_time($mongo_time)
	{
		$time_array = explode(' ' , $mongo_time);
		
		if(sizeof($time_array) > 1)
		{
			$r = $time_array[1];
		}
		else 
	...
	*/
	...
	public function update($collection = '', $options = array())
	{
		...
		try
		{
			// ofer $options = array_merge(array($this->_query_safety => TRUE, 'multiple' => FALSE), $options);
			$options = array_merge(array("w" => 1, 'multiple' => FALSE), $options);
	...
	public function update_all($collection = '', $options = array())
	{
		...
		try
		{
			// ofer $options = array_merge(array($this->_query_safety => TRUE, 'multiple' => TRUE), $options);
			$options = array_merge(array("w" => 1, 'multiple' => TRUE), $options);
	...
	public function delete($collection = '')
	{
		...
		try
		{
			...
			//$this->_dbhandle->{$collection}->remove($this->wheres, array("w"=>1, 'justOne' => TRUE));
	...
	public function delete_all($collection = '')
	{
		...
		try
		{
			// ofer $this->_dbhandle->{$collection}->remove($this->wheres, array($this->_query_safety => TRUE, 'justOne' => FALSE));
			$this->_dbhandle->{$collection}->remove($this->wheres, array("w" => 1, 'justOne' => FALSE));
	...
	private function _connect()
	{
		...
		try
		{
			// ofer $this->_connection = new Mongo($this->_connection_string, $options);
	...
	{		
		...
		// ofer $this->_query_safety = trim($this->_config_data['mongo_query_safety']);
